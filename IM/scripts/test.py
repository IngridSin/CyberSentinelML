...
# Load dataset
new_df = pd.read_csv("../Data/CEAS_08.csv")  # Update with the actual dataset path

# Extract relevant features and target
new_df = new_df[['sender', 'subject', 'body', 'label', 'urls']]
...
new_df = new_df[new_df['body'].str.strip() != ""] # Remove both empty strings and strings with only spaces
new_df = new_df.dropna(subset=['body']) # Remove rows that is NaN

# Remove empty subject row
new_df = new_df.dropna(subset=['subject']) # Remove rows that is NaN

# Remove empty label row
new_df = new_df.dropna(subset=['label']) # Remove rows that is NaN
...
new_df['sender_encoded'] = label_encoder.fit_transform(new_df['sender'])
...
new_df['cleaned_text_subject'] = new_df['subject'].apply(clean_text)
new_df['cleaned_text_body'] = new_df['body'].apply(clean_text)

# Process 'subject' and 'body' using TF-IDF
tfidf_subject = TfidfVectorizer()
tfidf_body = TfidfVectorizer()

X_subject = tfidf_subject.fit_transform(new_df['cleaned_text_subject'])
X_body = tfidf_body.fit_transform(new_df['cleaned_text_body'])
X_sender = new_df['sender_encoded'].values.reshape(-1, 1)
